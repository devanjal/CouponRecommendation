var getCouponList = angular.module('getCouponList', []);
//var Chart = require('src/chart.js')

getCouponList.controller('getCouponList', function($scope, $http, $filter){
	// $http({
	// 	method : "GET",
	// 	url : '/getCouponList',
	...
	$scope.searchUserName = function(){
		...
		$http({
			...
			url : '/getCouponList',
				...
				$scope.items = [];
				response.finalResult.forEach(function(element){
  ...
  $scope.getDataFromDB=function(){
    ...
    $scope.items = [];
    tempArrayFromDB.forEach(function(element){
      $scope.items.push(element);
    });
...
getCouponList.$inject = ['$scope', '$filter'];

getCouponList.directive("customSort", function() {
return {
    restrict: 'A',
    transclude: true,
    scope: {
